...
export class Atualizacao71777034476970 implements MigrationInterface {
    name = 'Atualizacao71777034476970'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`lista_rota\` ADD \`dataReferencia\` timestamp NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`lista_rota\` ADD \`tipo_lista\` varchar(255) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`atribuicao_final\` ADD \`listaRotaId\` int NULL`);
        await queryRunner.query(`ALTER TABLE \`passageiro\` ADD \`corridaSolicitadaId\` int NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`passageiro\` DROP COLUMN \`corridaSolicitadaId\``);
        await queryRunner.query(`ALTER TABLE \`atribuicao_final\` DROP COLUMN \`listaRotaId\``);
        await queryRunner.query(`ALTER TABLE \`lista_rota\` DROP COLUMN \`tipo_lista\``);
        await queryRunner.query(`ALTER TABLE \`lista_rota\` DROP COLUMN \`dataReferencia\``);
    }
