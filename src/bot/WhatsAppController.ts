...
export class WhatsAppController {
    ...
    if (comando.startsWith('@add') && !comando.includes(' posição ')) {
        ...
        try {
            ...
            await this.enviarMensagemComBox(this.grupoId, "Sucesso", `✅ *${nomeExibicao}* adicionado ao final da fila.`);
        } catch (error: any) {
            await this.enviarMensagemComBox(this.grupoId, "Erro", error.message);
        }
        return true;
    ...
    if (comando.startsWith('@inserir')) {
        const mencionados = await msg.getMentions();
        const contato = mencionados[0];
        const posicao = parseInt(comando.split(' posição ')[1]);

        if (!contato || isNaN(posicao)) {
            await this.enviarMensagemComBox(this.grupoId, "Erro", "Use: @inserir @Nome posição 3");
            return true;
        }
        ...
        try {
            ...
            await this.enviarMensagemComBox(this.grupoId, "Sucesso", `✅ *${nomeExibicao}* inserido na posição ${posicao}.`);
        } catch (error: any) {
            await this.enviarMensagemComBox(this.grupoId, "Erro", error.message);
        }
        return true;
    ...
    if (comando.startsWith('@remover')) {
        ...
        try {
            ...
            await this.enviarMensagemComBox(this.grupoId, "Sucesso", `❌ *${nomeExibicao}* foi removido da lista.`);
        } catch (error: any) {
            await this.enviarMensagemComBox(this.grupoId, "Erro", error.message);
        }
        return true;
